...
inline unsigned softmax_stable_idx_from_real_val(const data_T x) {
    // Treat the underlying bits of the fixed-point variable as the unsigned index.
    // This perfectly matches the "i * 2^-F" logic used in your Python script.
...
void softmax_stable(const data_T &data, res_T &res) {
    // Look-up tables generated from your Python script
    #include "activation_tables/exp_table.tb"
    #include "activation_tables/invert_table.tb"

    // 1. Find maximum using the input's value type
    ...
    // 2. Normalize inputs: d = x_max - x
    // CHANGE: Use CONFIG_T::inp_norm_t (ac_fixed<3,1,false...>)
    ...
    // 3. Exponentials
    // CHANGE: Use CONFIG_T::exp_table_t (ac_fixed<5,2,false...>)
    [[intel::fpga_register]] typename CONFIG_T::exp_table_t exp_res[CONFIG_T::n_in];
    #pragma unroll
    for (unsigned i = 0; i < CONFIG_T::n_in; i++) {
        // Use the index helper with the normalized input type
    ...
    // CHANGE: Use CONFIG_T::accum_t (ac_fixed<11,4,false>) for the adder tree
    ...
    // 5. Reciprocal of Sum
    // CHANGE: Cast exp_sum to inv_inp_t (ac_fixed<4,2,false...>) before table lookup
    ...
    // CHANGE: Use CONFIG_T::inv_table_t (ac_fixed<6,2,false...>)
    [[intel::fpga_register]] typename CONFIG_T::inv_table_t inv_exp_sum = invert_table[inv_idx];

    // 6. Final Multiplication
    #pragma unroll
    for (unsigned i = 0; i < CONFIG_T::n_in; i++) {
        // Result is stored in res_T (result_t: ac_fixed<11,4,false>)
...
// *************************************************
//       Multidimensional Softmax (Nested Loop Fix)
// *************************************************
...
inline void softmax_multidim(const data_T &data, res_T &res) {
    ...
    // Iterate through the 3D structure explicitly to guarantee index alignment
    #pragma unroll
    for (unsigned i = 0; i < CONFIG_T::n_outer; i++) {
        ...
        for (unsigned k = 0; k < CONFIG_T::n_inner; k++) {
            ...
            for (unsigned j = 0; j < CONFIG_T::n_slice; j++) {
                // Standard 3D flattened index
                unsigned idx = (i * CONFIG_T::n_slice * CONFIG_T::n_inner) + 
                               (j * CONFIG_T::n_inner) + 
                               k;
