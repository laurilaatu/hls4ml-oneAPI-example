...
int main(int argc, char **argv) {
    ...
    if (fin.is_open() && fpr.is_open()) {
        ...
        for (; std::getline(fin, iline) && std::getline(fpr, pline); iteration++) {
            ...
            nnet::convert_data<float, Conv1D3InputPipe, N_INPUT_1_1*N_INPUT_2_1>(q, vals);

            q.single_task(Myproject{});

            // hls-fpga-machine-learning convert output
            float outputs[N_OUTPUTS_7*N_FILT_7];
            nnet::convert_data_back<Layer7OutPipe, float, N_OUTPUTS_7*N_FILT_7>(q, outputs);
